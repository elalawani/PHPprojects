...
@section('content')
    ...
    <div class=" container m-auto pt-15 pb-5">
        ...
        @if(\Illuminate\Support\Facades\Auth::user() && \Illuminate\Support\Facades\Auth::user()->id == $post->user_id)
        <a href="/blog/{{$post->slug}}/edit"
           class="bg-green-700 text-green-100
           py-4 px-5 mt-5 inline-block rounded-lg
           font-bold uppercase text-l place-self-start">Edit Post</a>

            <form action="/blog" method="post" class="inline-block">
                @csrf
                @method('delete')
                <button class="bg-red-900 text-red-100
                               py-4 px-5 mt-5 inline-block rounded-lg
                               font-bold uppercase text-l place-self-start"
                >
                    Delete Post
                </button>
            </form>
